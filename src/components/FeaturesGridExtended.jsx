...
import './FeaturesGridExtended.css';
import './FeaturesGrid.css'; // Reuse core card styles
...
const FeaturesGridExtended = () => {
  return (
    <section id="features-grid" className="features-grid-ext-section animate-on-scroll fade-up">
      <div className="container">
        {featureGroups.map((group, groupIdx) => (
          <div key={group.label} className="fg-ext-group">
            <div className="fg-ext-group-header">
              <span className="fg-ext-group-badge">{group.label}</span>
            </div>
            <div className="fg-ext-grid animate-on-scroll fade-up">
              {group.features.map((f, idx) => {
                const staggerClass = idx % 4 === 0 ? '' : idx % 4 === 1 ? 'delay-100' : idx % 4 === 2 ? 'delay-200' : 'delay-300';
                return (
                  <div key={f.id} className={`fg-card-wrapper animate-on-scroll fade-up ${staggerClass}`}>
                    <GlassCard id={f.id} className="fg-card hover-glow">
...
export default FeaturesGridExtended;
